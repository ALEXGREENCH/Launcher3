...
public class Workspace extends ViewGroup implements DropTarget, DragSource, DragScroller {
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            setScrollX(currentScrollX + (int)(dx * e));
            mSmoothingTime = now;
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (xMoved || yMoved) {
                    ...
                    if (yMoved && y < mLastMotionY) {

                        float currentY = ev.getY();
                        float deltaY = currentY - mLastMotionY;
                        float minSwipeDistance = 100;
                        if (Math.abs(deltaY) > minSwipeDistance) {
                            Log.d(TAG, "bottom2top swipe");
                            menuOpenListener.openMenu();
                        }
    ...
    MenuOpenListener menuOpenListener;

    void setListenerOpenMenu(MenuOpenListener listener){
